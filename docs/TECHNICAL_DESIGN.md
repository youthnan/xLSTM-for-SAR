...
六旋翼无人机搭载 SAR（合成孔径雷达，f=9.6 GHz，v≈10 m/s，h≈300 m，斜距≈1000 m）执行对地成像任务时，导航系统（GPS/IMU）输出的飞行轨迹 $P_{raw}$ 含有积累性误差，导致 SAR 图像散焦。
...
### P0（仿真对齐实测）
- [ ] 更新 `data_generate_v4.m`：飞行高度从 60–140 m → 280–320 m，斜距约束 ~1000 m
